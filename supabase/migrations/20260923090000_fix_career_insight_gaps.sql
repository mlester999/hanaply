-- Fix: career insights never found a skill gap.
--
-- `public.career_insights` detected repeated gaps with
-- `entry ->> 'status' = 'missing'`, but the matching engine only ever emits
-- `met`, `partially_met`, `unmet`, or `unknown` (see
-- `packages/matching/src/index.ts` and `jobRequirementMappingSchema`). No
-- requirement mapping could ever carry the value `missing`, so the `gaps` array
-- was always empty and the `address_gaps` coaching suggestion could never fire.
--
-- The original pgTAP suite did not catch it because its only gap assertion was
-- that a single repeated gap does *not* yet justify a suggestion, which passed
-- for the wrong reason. `150_career_insights.test.sql` now also asserts that two
-- opportunities sharing an unmet requirement *do* produce a gap.
--
-- `unmet` and `partially_met` both count: a requirement the subscriber only
-- partly meets is still worth either closing or deliberately targeting around.
-- `unknown` is excluded, because that means the engine could not tell rather
-- than that the subscriber falls short.
    ...
    'gaps', coalesce((
      ...
      from (
        select
          ...
          -- The statuses are aggregated so the interface can distinguish a
          -- requirement that is unmet outright from one that is partly met.
          pg_catalog.string_agg(distinct entry ->> 'status', ', ' order by entry ->> 'status')
            as met_status
        from public.job_matches as matches
        ...
        where matches.career_profile_id = profile_id
          ...
          and entry ->> 'status' in ('unmet', 'partially_met')
...
comment on function public.career_insights(uuid, uuid, integer) is
  'Deterministic coaching and analytics for one career profile. A gap is a requirement whose mapping status is unmet or partially_met across two or more opportunities; unknown is excluded because that means the engine could not tell. Every rate is null rather than zero when its denominator is zero, and every suggestion carries the count that produced it.';
