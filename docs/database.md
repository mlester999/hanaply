...
The career, job intelligence, matching, radar, and application-pack domains add eleven more:
...
28. `20260920090000_admin_job_operations.sql` - provider governance with an audited reason, ingestion health, the internal job directory, and deduplication diagnostics for operators.
29. `20260921090000_career_insights_and_coaching.sql` - deterministic career insights and ranked coaching. Every rate is null rather than zero when its denominator is zero, and every suggestion carries the count that produced it.
30. `20260922090000_application_pack_generation.sql` - `app_private.require_application_pack_owner`, the generation context and finalisation entry point `public.generate_application_pack_artifacts`, and a `public.complete_application_pack` that refuses to mark a pack ready while it has no artifact. It reads the confirmed evidence the generator may cite from the intersection of the facts confirmed now and the identifiers frozen onto the pack, so a fact confirmed after pack creation is never cited behind the subscriber's back.
