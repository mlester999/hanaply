...
- **Application Pack artifacts** are `generatePackArtifacts` in `services/api/src/pack-generation.ts`, added with `20260922090000_application_pack_generation.sql`. It takes the career profile, the confirmed career facts the pack froze, the posting, and the frozen match snapshot, and returns one draft per requested kind. It reads no clock, performs no I/O, and uses no randomness, so identical inputs produce byte-identical drafts. Every fact it quotes is quoted verbatim, `evidenceFactIds` lists exactly the facts it quoted, and the only numbers that can appear in its output already exist in the inputs: it never computes, rounds, totals, or scales a figure. When a requirement has no confirmed evidence it says so rather than paraphrasing one into existence. Interview preparation, recruiter messages, cover letters, tailored resumes, and the weekly career strategy therefore have a generator; what they do not have is model-written prose.
